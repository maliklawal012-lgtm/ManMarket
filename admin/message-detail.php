...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_status'])) {
    ...
    header('Location: /market/admin/message-detail?id=' . $messageId);
    exit;
}
...
if (!$message) {
    header('Location: /market/admin/messages');
    exit;
}
...
<div class="admin-toolbar" style="margin-bottom: var(--gap);">
    <a href="/market/admin/messages" class="link-more"><?= icon('chevron-right', 14) ?> Retour aux messages</a>
</div>
...
<div class="card" style="margin-bottom: var(--gap);">
    <div class="admin-toolbar">
        ...
        <div class="admin-table-actions">
            ...
            <form method="post" action="/market/admin/message-detail?id=<?= $messageId ?>">
                <?= csrf_field() ?>
    ...
    <ul class="account-info-list">
        <li><span class="account-info-label"><?= icon('user', 16) ?> Expéditeur</span><span>
            <?= e($message['name']) ?>
            <?php if ($message['user_id']): ?><br><span class="char-count"><a href="/market/admin/utilisateurs?action=edit&id=<?= (int) $message['user_id'] ?>" class="link-muted"><?= e($message['user_name'] ?? $message['user_account_email']) ?></a> — compte lié</span><?php endif; ?>
        </span></li>
        ...
        <?php if ($message['shop_id']): ?>
            <li><span class="account-info-label"><?= icon('store', 16) ?> Boutique concernée</span><span><a href="/market/admin/boutique-detail?id=<?= (int) $message['shop_id'] ?>" class="link-muted"><?= e($message['shop_name']) ?></a></span></li>
        <?php endif; ?>
            ...
            <div class="admin-activity-list">
                <?php foreach ($otherByCustomer as $m): ?>
                    <div class="admin-activity-item">
                        ...
                        <div>
                            <div class="admin-activity-text"><a href="/market/admin/message-detail?id=<?= (int) $m['id'] ?>" class="link-muted"><?= e($m['subject']) ?></a> — <?= e(mb_strimwidth($m['message'], 0, 60, '…')) ?></div>
            ...
            <div class="admin-activity-list">
                <?php foreach ($otherByShop as $m): ?>
                    <div class="admin-activity-item">
                        <span class="admin-activity-icon"><?= icon('send', 15) ?></span>
                        <div>
                            <div class="admin-activity-text"><a href="/market/admin/message-detail?id=<?= (int) $m['id'] ?>" class="link-muted"><?= e($m['name']) ?></a> — <?= e(mb_strimwidth($m['message'], 0, 60, '…')) ?></div>
                            <div class="admin-activity-time"><span class="tag <?= $m['status'] === 'pending' ? 'tag-pending' : 'tag-green' ?>"><?= $m['status'] === 'pending' ? 'Non traité' : 'Traité' ?></span> · <?= e(date('d/m/Y', strtotime((string) $m['created_at']))) ?></div>
                        </div>
                    </div>
                <?php endforeach; ?>
