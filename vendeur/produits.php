...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if (!$errors) {
        ...
        if ($id > 0) {
            ...
            $stmt = $db->prepare('
                INSERT INTO products (name, slug, description, category_id, shop_id, price, original_price, stock, size_type, icon, image, is_featured)
                VALUES (:name, :slug, :description, :category_id, :shop_id, :price, :original_price, :stock, :size_type, :icon, :image, :is_featured)
            ');
            $stmt->execute([
...
if ($formAction === 'new') {
    $editing = [
        ...
        'price' => '', 'original_price' => '', 'stock' => 0, 'size_type' => 'none', 'icon' => '', 'image' => null, 'is_featured' => 0,
    ];
            ...
            <script>
                (function () {
                    ...
                    function refresh() {
                        ...
                        stockGlobalField.style.display = type === 'none' ? '' : 'none';
                    }
