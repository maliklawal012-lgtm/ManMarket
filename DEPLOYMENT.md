...
**Mot de passe HTTP en amont de `/admin/*`** — ✅ déjà en place sur cet environnement (19 août 2026), via `admin/.htaccess` (Basic Auth Apache). Barriere supplementaire *avant meme* d'atteindre la page de connexion : un visiteur doit connaitre ce mot de passe HTTP en plus d'avoir un vrai compte admin + le code 2FA. Le fichier `AuthUserFile` est place hors du dossier web servi (`C:/wamp64/.htpasswd-manmarket-admin`, jamais accessible en HTTP), genere avec `htpasswd.exe -B` (bcrypt). **Chemin specifique a cet environnement WAMP** — sur un nouveau serveur, regenerer le fichier (`htpasswd -cB <chemin_hors_docroot> <utilisateur>`) et adapter `AuthUserFile` dans `admin/.htaccess` en consequence. Identifiants communiques separement (jamais commites).
...
**Vérifié avant migration (19 août 2026)** : aucun chemin Windows codé en dur dans le code PHP, aucune incohérence de casse de fichier (501 `require`/`include` contrôlés — un problème fréquent en passant de Windows, insensible à la casse, à Linux, qui y est strictement sensible), `site_base_url()` s'adapte déjà automatiquement au nom de domaine réel via `$_SERVER['HTTP_HOST']`. Le code lui-même n'a besoin d'aucune modification pour ce changement d'hébergeur, **à condition de respecter le point 1 ci-dessous**.

⚠️ **Obligatoire : déployer dans un sous-dossier `market/`, jamais à la racine `public_html/`.** Le code code en dur `/market/` à plus de 500 endroits (liens, `header('Location: ...')`, assets, `site_base_url()` dans `includes/functions.php`, `ErrorDocument 404 /market/404.php` et les règles `RewriteCond ... !^/market/api/` de `.htaccess`). Un dépôt directement à la racine du domaine casse tout le site (liens et assets en 404, page 404 elle-même introuvable) et, plus grave, désactive l'exclusion du webhook Genius Pay (la condition `!^/market/api/` ne matche plus une URI réelle `/api/...`), ce qui peut casser la réconciliation des paiements en production.

1. **Transférer le code** — via SSH (`git clone`/`git pull`, recommandé pour les mises à jour futures) ou le Gestionnaire de fichiers cPanel. Placer le contenu de ce dépôt dans `public_html/market/` (créer ce sous-dossier s'il n'existe pas) — **jamais directement dans `public_html/`**. Le site sera alors accessible sur `https://votre-domaine/market/...`, conformément à tous les chemins codés en dur dans le code.
